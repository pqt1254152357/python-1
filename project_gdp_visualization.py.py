...
def reconcile_countries_by_name(plot_countries, gdp_countries):  # 返回在世行有GDP数据的绘图库国家代码字典，以及没有世行GDP数据的国家代码集合
    ...
    for i in plot_countries:
        ...
        result = country_name in gdp_countries#利用if函数筛选出筛选出pygal库国家信息中有世行GDP数据的国家以及没有GDP数据的国家，并分别存入dict1，set1
        if result == True:
...
def build_map_dict_by_name(gdpinfo, plot_countries, year):
   ...
   """
    ...
    dict1 = tuple1[0]#将tuple1【0】赋值给dict1
    set1 = tuple1[1]
    set2 = set()
    dict2 = {}
    for i in dict1:# 筛选出某一给定年份各国及其在世行对应的GDP值、没有世行GDP数据的国家以及只是该年没有GDP数据的国家
        countryname = dict1[i]
        ...
        if value1 == "":
            ...
            value1 = float(value1)
            value1 = math.log10(value1)#进行对数转换
            dict2[i] = value1
    tuple2 = (dict2, set1, set2)#将结果转化为一个字典和二个集合的元组数据
    return (tuple2)#返回结果
...
def render_world_map(gdpinfo, plot_countries, year, map_file):  # 将具体某年世界各国的GDP数据(包括缺少GDP数据以及只是在该年缺少GDP数据的国家)以地图形式可视化
    ...
    gdp_world_map.title = "全球GDP分布图"#输出具体的图片
    gdp_world_map.add(year, dict3)
    gdp_world_map.add("missing from world bank", set1)#生成可视化结果
